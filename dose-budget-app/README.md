...
| `LOCK_TIMEOUT_MS` | `10000` | 多进程共享数据文件时等待排他锁的最长时间，超时返回 `503 lock_timeout`（本次写入不执行，旧数据不变） |
| `LOCK_STALE_MS` | `15000` | 锁文件超过该年龄且持锁进程已不在本机时才被接管 |

浏览器打开 `http://localhost:8080` 即可使用。

## 多进程并发安全

多个服务进程可以指向同一个 `DATA_FILE`。每个写事务执行 **跨进程排他文件锁 → 锁内从磁盘重读最新状态 → 应用变更 → `fsync` + 原子 `rename` → 释放锁**（见 `server/lock.js`、`server/storage.js`）；每个读请求处理前也会先从磁盘重读。因此：

- 并发写入不会"最后写入者获胜"：所有有效记录都保留，编号由锁内计数器串行分配，**唯一且连续、不重复**；
- 相同 `source_ref` 的并发竞态也由锁 + 唯一约束仲裁，恰好一次成功，其余返回 `409 duplicate_source_ref`；
- 任一步失败（业务校验、临时文件写失败、`rename` 失败、等锁超时）都回滚且**不破坏旧数据**；锁只在"超龄且持锁进程已死亡"时接管，绝不抢占活锁。
...
测试稳定区分通过/失败（断言失败即非零退出），可重复运行：每个文件使用独立的临时数据目录，`after/afterEach` 钩子结束时删除临时文件，互不共享状态。

- `test/dosimetry.test.js`：周期边界、单位换算、更正链、风险带顺序、首超、五年平均的表驱动单测；
- `test/budget-selection.test.js`：评估页人员↔计划选择调和——A 选计划→切 B 自动清空→**切回 A 原计划重新可选**→刷新后计划删除/改挂时清空→提交参数始终自洽；
- `test/storage.test.js`：存储层事务与持久性——连续写 30 人不丢、40 条并发写经写队列串行化不重号、**注入临时文件写入/rename 失败后内存回滚且磁盘旧数据可回读**、业务校验失败不留痕、更正事务失败不留半条链、评估只追加且快照不变、审计只增且存储层无删除方法、**双 opener 一方写失败不波及另一方/交错 40 写编号连续/等锁超时 503/陈旧锁接管**；
- `test/multiprocess.test.js`：**两个真实服务进程共享同一数据文件**——同时各写 20 人全部 201 且 40 条不丢、编号唯一连续；同一 `source_ref` 五连并发恰好一次成功；两进程交错写暴露/计划不冲突；第三打开器回读完整、计数器连续、审计含两个操作者；
- `test/multiprocess-crash.test.js`：**崩溃与压力**——持锁进程被 `SIGKILL` 后锁超龄且进程不存在，新进程接管陈旧锁写入成功（编号连续、旧数据完整）；两进程高频交错并发写 24 人员 + 24 暴露 + 24 计划全部 201、三类实体编号全局唯一连续；锁被存活进程长期持有时等锁请求收到明确 `503 lock_timeout`、旧数据不变、释放后恢复（用 `scripts/lock-holder.js` 模拟持锁/崩溃）。
- `test/storage-integration.test.js`：真实 HTTP 服务——连续跨实体写入不丢、评估只追加、对审计/评估/各实体的 DELETE/PUT/PATCH 无移除入口（404/405）且数据不变、审计倒序与过滤；
- `test/e2e.test.js`：主流程建限值→录入→重复来源拒绝→核验→更正保留原记录→计划→评估/首超/风险带→**跨人员计划被 `cross_worker` 拒绝**→保存→越级拒绝→RPO 人工接受→审计→**杀进程重启后从磁盘完整回读**。
...
│   ├── lock.js        # 跨进程排他文件锁（O_EXCL、退避、超时、陈旧锁接管）
│   ├── storage.js     # 锁内重读-修改-原子提交的 JSON 存储、唯一约束、更正事务、审计
